...
import { ConferenceDay, Speaker, RoomInfo, Session, FavoriteSession } from '../../shared/interfaces';
...
class MainViewModel extends Observable {
    ...
    private pushSessions(sessionsFromService: Array<Session>) {
        for (var i = 0; i < sessionsFromService.length; i++) {
            ...
            var indexInFavs = favoritesServiceModule.findSessionIndexInFavorites(newSession.id);
            if (indexInFavs >= 0) {
                newSession.favorite = true;
                newSession.calendarEventId = favoritesServiceModule.favorites[indexInFavs].calendarEventId;
            }
